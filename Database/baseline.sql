--
-- File generated with SQLiteStudio v3.1.0 on Tue May 15 12:16:29 2018
--
...
CREATE TABLE sched_disclaimer (
    disclaimer_id   INTEGER      PRIMARY KEY AUTOINCREMENT,
    disclaimer_desc VARCHAR (20) NOT NULL ON CONFLICT ROLLBACK
                                 UNIQUE ON CONFLICT ROLLBACK,
    disclaimer_text TEXT         NOT NULL ON CONFLICT ROLLBACK
);


-- Table: sched_freightHoliday
CREATE TABLE sched_freightHoliday (
    freightHoliday_id   INTEGER      PRIMARY KEY AUTOINCREMENT,
    freightHoliday_date DATE         NOT NULL ON CONFLICT ROLLBACK,
    freightHoliday_desc VARCHAR (20) 
);


-- Table: sched_DateType
CREATE TABLE sched_DateType (
    dateType_id             INTEGER      PRIMARY KEY AUTOINCREMENT,
    fk_groups_id            INTEGER      REFERENCES [sched-Groups] (groups_id) ON UPDATE CASCADE,
    dateType_Name           VARCHAR (20) UNIQUE ON CONFLICT ROLLBACK
                                         NOT NULL ON CONFLICT ROLLBACK,
    dateType_Weekend        BOOLEAN      DEFAULT (0) 
                                         NOT NULL ON CONFLICT ROLLBACK,
    dateType_PlantHoliday   BOOLEAN      DEFAULT (0) 
                                         NOT NULL ON CONFLICT ROLLBACK,
    dateType_FreightHoliday BOOLEAN      NOT NULL ON CONFLICT ROLLBACK
                                         DEFAULT (0),
    dateType_USPSHoliday    BOOLEAN      NOT NULL ON CONFLICT ROLLBACK
                                         DEFAULT (0) 
);
...
CREATE TABLE Modules (
    ...
    ModuleCode            TEXT (2) NOT NULL ON CONFLICT ROLLBACK
);
...
CREATE TABLE sched_frequency (
    frequency_id   INTEGER      PRIMARY KEY AUTOINCREMENT,
    frequency_desc VARCHAR (20) NOT NULL ON CONFLICT ROLLBACK,
    frequency_days INTEGER      NOT NULL ON CONFLICT ROLLBACK
);
...
CREATE TABLE sched_Groups (
    ...
    groups_name VARCHAR (20) UNIQUE ON CONFLICT ROLLBACK
                             NOT NULL ON CONFLICT ROLLBACK
);
...
CREATE TABLE ShipVia (
    ...
    CarrierID        INTEGER REFERENCES Carriers (Carrier_ID) ON DELETE CASCADE
                                                              ON UPDATE CASCADE,
    CarrierPkgID     INTEGER REFERENCES CarrierPkg (CarrierPkg_ID),
...
CREATE TABLE sched_PlantHoliday (
    plantHoliday_id   INTEGER      PRIMARY KEY AUTOINCREMENT,
    plantHoliday_date DATE         NOT NULL ON CONFLICT ROLLBACK,
    plantHoliday_desc VARCHAR (20) 
);


-- Table: sched_uspsHoliday
CREATE TABLE sched_uspsHoliday (
    uspsHoliday_id   INTEGER      PRIMARY KEY AUTOINCREMENT,
    uspsHoliday_date DATE         NOT NULL ON CONFLICT ROLLBACK,
    uspsHoliday_desc VARCHAR (20) 
);
...
CREATE TABLE SecurityAccess (
    ...
    SecGrpNameID     INTEGER REFERENCES SecGroupNames (SecGroupName_ID) ON DELETE CASCADE
                                                                        ON UPDATE CASCADE
                             NOT NULL ON CONFLICT ROLLBACK,
    ModID            INTEGER REFERENCES Modules (Mod_ID) ON DELETE CASCADE
                                                         ON UPDATE CASCADE
                             NOT NULL ON CONFLICT ROLLBACK,
    SecAccess_Read   BOOLEAN DEFAULT (0) 
                             NOT NULL ON CONFLICT ROLLBACK,
    SecAccess_Write  BOOLEAN DEFAULT (0) 
                             NOT NULL ON CONFLICT ROLLBACK,
    SecAccess_Delete BOOLEAN NOT NULL ON CONFLICT ROLLBACK
                             DEFAULT (0) 
);
...
CREATE TABLE sched_workflow (
    ...
    workflow_desc        VARCHAR (20) NOT NULL ON CONFLICT ROLLBACK
                                      UNIQUE ON CONFLICT ROLLBACK,
    workflow_primaryDate BOOLEAN      NOT NULL ON CONFLICT ROLLBACK
                                      DEFAULT (0),
    workflow_offset      INTEGER (1)  NOT NULL ON CONFLICT ROLLBACK
                                      DEFAULT (0),
    fk_dateType_id       INTEGER      REFERENCES [sched-DateType] (dateType_id) ON UPDATE CASCADE
);
...
CREATE TABLE Schema (
    ...
    SchemaVers  TEXT,
    FileName    TEXT,
    UserID      INTEGER REFERENCES Users (User_ID) ON UPDATE CASCADE
);
...
-- Table: LabelVersions
CREATE TABLE LabelVersions (
    labelVersionID   INTEGER PRIMARY KEY AUTOINCREMENT,
    tplID            INTEGER REFERENCES LabelTPL (tplID) ON DELETE CASCADE
                             NOT NULL,
    LabelVersionDesc TEXT    NOT NULL ON CONFLICT ROLLBACK
);
...
-- Table: CarrierPkg
CREATE TABLE CarrierPkg (
    CarrierPkg_ID  INTEGER      PRIMARY KEY ASC AUTOINCREMENT,
    CarrierID      INTEGER      NOT NULL ON CONFLICT ROLLBACK
                                REFERENCES Carriers (Carrier_ID),
    CarrierPkgDesc VARCHAR (25) NOT NULL ON CONFLICT ROLLBACK
);
...
CREATE TABLE LabelData (
    ...
    labelVersionID INTEGER REFERENCES LabelVersions (labelVersionID) ON UPDATE CASCADE
                           NOT NULL ON CONFLICT ROLLBACK,
...
CREATE TABLE LabelHeaderGrp (
    LabelProfileID INTEGER REFERENCES LabelProfiles (LabelProfileID),
    LabelHeaderID  INTEGER REFERENCES LabelHeaders (LabelHeaderID) 
);
...
-- Table: LabelHeaders
CREATE TABLE LabelHeaders (
    LabelHeaderID         INTEGER PRIMARY KEY AUTOINCREMENT,
    LabelHeaderDesc       TEXT    NOT NULL ON CONFLICT ABORT
                                  UNIQUE ON CONFLICT ROLLBACK,
    LabelHeaderSystemOnly BOOLEAN DEFAULT (0) 
);
...
CREATE TABLE HeadersConfig (
    ...
    dbActive                BOOLEAN NOT NULL ON CONFLICT ROLLBACK
                                    DEFAULT (1),
    CopyColumn              BOOLEAN NOT NULL ON CONFLICT ROLLBACK
                                    DEFAULT (0) 
);
