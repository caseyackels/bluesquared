...
proc eAssistSetup::admin_GUI {args} {
    ...
    #
    #***
    global log G_setupFrame program admin wid widSec widTmp sec sysdb
    ...
    $nbk add [ttk::frame $nbk.integrations] -text [mc "Integrations"]
    ...
    ## **********************
    ## Tab 3 (Integrations)
    ## A place to enter the path to the Monarch db, user id and pwd. For use during connections in various parts of the program.
    ##

    ## THe Monarch DB is actually a database that holds views. We aren't actually accessing the Monarch DB itself.

    set wid(integrations,f1) [ttk::frame $nbk.integrations.f1 -padding 10]
    pack $wid(integrations,f1) -expand yes -fill both

# Server Name
# Database
# Userid / user pwd
    grid [ttk::label $wid(integrations,f1).txt1 -text [mc "Server Name"]] -column 0 -row 0 -padx 2p -pady 2p -sticky w
    grid [ttk::entry $wid(integrations,f1).entry1 -textvariable sysdb(serverName)] -column 1 -row 0 -padx 2p -pady 2p -sticky ew

    grid [ttk::label $wid(integrations,f1).txt2 -text [mc "Database"]] -column 0 -row 1 -padx 2p -pady 2p -sticky w
    grid [ttk::entry $wid(integrations,f1).entry2 -textvariable sysdb(database)] -column 1 -row 1 -padx 2p -pady 2p -sticky ew

    grid [ttk::label $wid(integrations,f1).txt3 -text [mc "User ID"]] -column 0 -row 2 -padx 2p -pady 2p -sticky w
    grid [ttk::entry $wid(integrations,f1).entry3 -textvariable sysdb(userid)] -column 1 -row 2 -padx 2p -pady 2p -sticky ew

    grid [ttk::label $wid(integrations,f1).txt4 -text [mc "Password"]] -column 0 -row 3 -padx 2p -pady 2p -sticky w
    grid [ttk::entry $wid(integrations,f1).entry4 -textvariable sysdb(userpwd)] -column 1 -row 3 -padx 2p -pady 2p -sticky ew

    grid [ttk::button $wid(integrations,f1).btn -text [mc "Save"] -command {ea::db::admin::saveDBInfo}] -column 1 -row 4 -padx 2p -pady 2p -sticky e
